...
YYCODETYPE   : constant := unsigned_char;
YYNOCODE     : constant := 8;
YYACTIONTYPE : constant := unsigned_char;
subtype ParseTOKENTYPE is int;
type YYMINORTYPE is record
   yyinit : Integer;
   yy0    : ParseTOKENTYPE;
end record;
pragma Unchecked_Union (YYMINORTYPE);
YYSTACKDEPTH : constant := 100;
type ParseARG_SDECL is null record;
type ParseARG_PDECL is null record;
type ParseARG_PARAM is null record;
procedure ParseARG_FETCH is null;
procedure ParseARG_STORE is null;
type ParseCTX_SDECL is null record;
type ParseCTX_PDECL is null record;
type ParseCTX_PARAM is null record;
procedure ParseCTX_FETCH is null;
procedure ParseCTX_STORE is null;
YYNSTATE             : constant := 8;
YYNRULE              : constant := 6;
YYNRULE_WITH_ACTION  : constant := 6;
YYNTOKEN             : constant := 6;
YY_MAX_SHIFT         : constant := 7;
YY_MIN_SHIFTREDUCE   : constant := 11;
YY_MAX_SHIFTREDUCE   : constant := 16;
YY_ERROR_ACTION      : constant := 17;
YY_ACCEPT_ACTION     : constant := 18;
YY_NO_ACTION         : constant := 19;
YY_MIN_REDUCE        : constant := 20;
YY_MAX_REDUCE        : constant := 25;
-------------- End control #defines --------------------------------------------
...
YY_ACTTAB_COUNT : constant := (16);
yy_action : constant array (Natural range <>) of YYACTIONTYPE := (
   --      0
      20,    3,    4,    1,    2,   18,    5,    1,    2,   24,
   --     10
      16,   19,   19,   23,    6,    7
   );
yy_lookahead : constant array (Natural range <>) of YYCODETYPE := (
   --      0
       0,    1,    2,    3,    4,    6,    7,    3,    4,    7,
   --     10
       5,    8,    8,    7,    7,    7,    8,    6,    6,    6,
...
YY_SHIFT_COUNT : constant := (7);
YY_SHIFT_MIN   : constant := (0);
YY_SHIFT_MAX   : constant := (5);
yy_shift_ofst : constant array (Natrual range <>) of unsigned_char := (
   --      0
       5,    5,    5,    5,    5,    0,    4,    4
   );
YY_REDUCE_COUNT : constant := (4);
YY_REDUCE_MIN   : constant := (-1);
YY_REDUCE_MAX   : constant := (8);
yy_reduce_ofst : constant array (Natural range <>) of signed_char := (
   --      0
      -1,    2,    6,    7,    8
   );
yy_default : constant array (Natural range <>) of YYACTIONTYPE := (
   --      0
      17,   17,   17,   17,   17,   17,   22,   21
);
...
yyRuleName : constant array (Natural range <>) of String := (
      "program ::= expr",  --    0
      "expr ::= expr MINUS expr",  --    1
      "expr ::= expr PLUS expr",  --    2
      "expr ::= expr TIMES expr",  --    3
      "expr ::= expr DIVIDE expr",  --    4
      "expr ::= INTEGER"   --    5
);
--#endif /* NDEBUG */
...
--  LINE calc_ada.y:21
begin
    Ada.Text_IO.Put_Line ("Result=" & YYmsp (0).Minor.YY0.Image);
end;
--  LINE calc_ada.adb:902

      when 1 =>  --  expr ::= expr MINUS expr
--  LINE calc_ada.y:25
begin
 YYlhsminor.YY0 := YYmsp (-2).Minor.YY0 - YYmsp (0).Minor.YY0;
end;
--  LINE calc_ada.adb:909
  YYmsp (-2).Minor.YY0 := YYlhsminor.YY0;

      when 2 =>  --  expr ::= expr PLUS expr
--  LINE calc_ada.y:29
begin
 YYlhsminor.YY0 := YYmsp (-2).Minor.YY0 + YYmsp (0).Minor.YY0;
end;
--  LINE calc_ada.adb:917
  YYmsp (-2).Minor.YY0 := YYlhsminor.YY0;

      when 3 =>  --  expr ::= expr TIMES expr
--  LINE calc_ada.y:33
begin
 YYlhsminor.YY0 := YYmsp (-2).Minor.YY0 * YYmsp (0).Minor.YY0;
end;
--  LINE calc_ada.adb:925
  YYmsp (-2).Minor.YY0 := YYlhsminor.YY0;

      when 4 =>  --  expr ::= expr DIVIDE expr
--  LINE calc_ada.y:37
begin
    if YYmsp (0).Minor.YY0 /= 0 then
	   YYlhsminor.YY0 := YYmsp (-2).Minor.YY0 / YYmsp (0).Minor.YY0;
    else
	   Ada.Text_IO.Put_Line ("YYlhsminor.YY0 divide by zero detected");
	end if;
end;
--  LINE calc_ada.adb:937
  YYmsp (-2).Minor.YY0 := YYlhsminor.YY0;
